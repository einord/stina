...
**Test count**: 496 tests pass (485 prior + 11 new policy route tests). **Typecheck**: clean across all packages and apps.
...
```sh
pnpm test                    # full suite, currently 495 tests
...
Should print 495 tests passed, no typecheck errors. If either fails, that's the regression to fix before doing anything else.

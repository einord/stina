...
- **What does NOT exist yet**: event-triggered threads (mail/calendar; no extension calls `emitEvent` yet). The §06 severity gate is now in place (high/critical tools blocked without a policy), but Escalate and Solve-differently collision options, policy creation UI, per-tool redactors, and severity-change cascade handling are all pending.
...
Should print 485 tests passed, no typecheck errors. If either fails, that's the regression to fix before doing anything else.

...
**Test count**: 520 tests pass (510 prior + 10 new pending-gate tests). **Typecheck**: clean across all packages and apps.
...
8. **Event-triggered threads from extensions** [L] — Phase 8a landed: `emitEvent` typed API on `ctx.events`, host trust-boundary validation (deny-list of `'user'`/`'stina'`, content/trigger kind pairing, `extension_id` stamping), thread spawn + decision turn end-to-end, dev-test extension exposes a `dev_test_emit_test_mail` driver tool. Phase 8b landed: failure mode framing (`applyFailureFraming` helper in `@stina/orchestrator`, `RUNTIME_EXTENSION_ID` constant in `@stina/core`, both apps wired). Phase 8c landed: pending-first-turn invisibility gate (`first_turn_completed_at` column, `markFirstTurnCompleted` repo method, `list()` default excludes pending, `runDecisionTurn` and `applyFailureFraming` lift the gate). **Remaining**: ~~failure mode~~ (landed in `188f162`), degraded mode (§04 line 147-157), ~~pending-first-turn UI invisibility gate~~ (landed in Phase 8c), notifications, `emitEventInternal`, linked-entity derivation from AppContent, closing-action normalization, scheduled-jobs migration to typed events, and per-extension migrations of mail/calendar/work to use the new API.

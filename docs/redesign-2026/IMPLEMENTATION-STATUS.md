...
2b. **Tool call severity rendering on persisted `StinaMessage.tool_calls`** [S, blocked] — depends on tool-call persistence landing first (the producer currently returns `{ visibility:'normal', content:{ text } }` without populating `tool_calls`). Once persistence lands, mirror the §05 visual weight onto the `InboxView.Message` rendering of Stina messages so historical tool calls in archived threads also render with severity-driven emphasis.
...
Should print 477 tests passed, no typecheck errors. If either fails, that's the regression to fix before doing anything else.

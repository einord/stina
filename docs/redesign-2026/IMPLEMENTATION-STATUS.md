...
- **What's wired but partial**: event-triggered threads — Phase 8a landed. The dev-test extension can spawn mail-triggered threads via `ctx.events.emitEvent` and the host runs Stina's decision turn against the new thread. Mail/calendar/work extensions still need rewrite to use the typed API; legacy `events.emit(name, payload)` coexists untouched. Failure mode, degraded mode, pending-first-turn invisibility, notifications, `emitEventInternal`, and linked-entity derivation are all deferred. The §06 severity gate + policy management UI are in place (users create/revoke policies via the "Autonomi" sidebar). The per-tool redactor pipeline is wired host-side, but extension-registered tools still hit the no-redactor branch because redactor functions can't cross the worker IPC boundary in v1. Remaining §06 gaps: Escalate and Solve-differently collision options, extension-IPC redactor support, severity-change cascade handling, and Stina-suggested policies.
...
| *(current)* | Phase 8a — emitEvent: typed event API + dev-test driver | `EmitEventInput`/`EmitEventResult`/`ExtensionThreadTrigger`/`ExtensionAppContent` added to `@stina/extension-api`. `EventsAPI.emitEvent` method available to all extensions (reuses `events.emit` permission). `EventsHandler` extended with `events.emitEvent` IPC path: deny-list rejects `user`/`stina` trigger kinds, enforces trigger↔content kind pairing, stamps `source.extension_id` + `trigger.extension_id` from host identity. `EmitThreadEventCallback` type wired through `ExtensionHostOptions` → `NodeExtensionHost` → `createNodeExtensionRuntime` → both `apps/api` and `apps/electron`. Callback creates thread, appends typed `AppMessage`, runs decision turn (fail-safe: errors logged, thread visible without Stina reply). `dev_test_emit_test_mail` tool (medium severity) added to dev-test extension alongside existing high-severity tool. 9 EventsHandler unit tests + 2 `emitEvent.test.ts` integration tests. |

**Test count**: 508 tests pass (497 prior + 9 new EventsHandler tests + 2 new emitEvent integration tests). **Typecheck**: clean across all packages and apps.
...
8. **Event-triggered threads from extensions** [L] — Phase 8a landed: `emitEvent` typed API on `ctx.events`, host trust-boundary validation (deny-list of `'user'`/`'stina'`, content/trigger kind pairing, `extension_id` stamping), thread spawn + decision turn end-to-end, dev-test extension exposes a `dev_test_emit_test_mail` driver tool. **Remaining**: failure mode + degraded mode (§04 line 137-157), pending-first-turn UI invisibility gate (§04 line 62-64), notifications, `emitEventInternal` (Stina-origin), linked-entity derivation from AppContent, closing-action normalization, scheduled-jobs migration to typed events, and per-extension migrations of mail/calendar/work to use the new API. Compile-time exhaustiveness on the trigger-kind switch (an unknown→typed-narrowing helper) is also an open follow-up — runtime enforcement is correct but the brief's "TS compile error on new kind" invariant isn't actually achieved at the IPC boundary.

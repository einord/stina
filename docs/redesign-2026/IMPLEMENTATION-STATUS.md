...
| _next_ | Phase 6a | SSE streaming server-side. `runDecisionTurn` accepts an `onStreamEvent` listener; both producers forward `content_delta` events through it, the orchestrator follows up with `message_appended` + `done` (or `error`). Two new endpoints — `POST /threads/stream` and `POST /threads/:id/messages/stream` — hijack the response and emit those events as `data: <json>\n\n` lines. The non-streaming endpoints are unchanged so existing UI keeps working until 6b lands. |

**Test count**: 391 tests pass (383 + 4 streaming tests in runDecisionTurn + 4 SSE route tests). **Typecheck**: clean across all packages and apps.
...
6. ~~**Real Stina with provider**~~ — landed in Phases 5b–6a. `DecisionTurnContext.memory` carries active standing instructions + linked-entity profile facts (5b); `createProviderProducer` assembles a system prompt + role-based `ChatMessage[]` and drains a `ChatStreamDispatcher` (5c); apps/api wiring resolves the user's default model config and dispatches via `ExtensionHost.chat()`, with safe fallback to the canned stub when no model is configured (5d); SSE streaming is wired server-side via new `POST /threads/stream` and `POST /threads/:id/messages/stream` endpoints that emit `content_delta`, `message_appended`, and `done` events to the client (6a). **Loose end remaining**: UI integration — the existing inbox UI still uses the non-streaming endpoints, so users see the reply land all at once even when the server streams. Phase 6b switches `useThreads.replyToSelected` + `createUserThread` to the streaming endpoints. Tool calls (`tool_start` / `tool_end`) are still ignored — see item 7.
...
Should print 391 tests passed, no typecheck errors. If either fails, that's the regression to fix before doing anything else.

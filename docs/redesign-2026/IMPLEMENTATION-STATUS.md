...
```sh
pnpm test                    # full suite, currently 508 tests
...
8. **Event-triggered threads from extensions** [L] — Phase 8a landed: `emitEvent` typed API on `ctx.events`, host trust-boundary validation (deny-list of `'user'`/`'stina'`, content/trigger kind pairing, `extension_id` stamping), thread spawn + decision turn end-to-end, dev-test extension exposes a `dev_test_emit_test_mail` driver tool. Phase 8b landed: failure mode framing (`applyFailureFraming` helper in `@stina/orchestrator`, `RUNTIME_EXTENSION_ID` constant in `@stina/core`, both apps wired). **Remaining**: ~~failure mode~~ (landed in `<commit-hash>`), degraded mode (§04 line 147-157), pending-first-turn UI invisibility gate (§04 line 62-64), notifications, `emitEventInternal`, linked-entity derivation from AppContent, closing-action normalization, scheduled-jobs migration to typed events, and per-extension migrations of mail/calendar/work to use the new API.
...
Should print 508 tests passed, no typecheck errors. If either fails, that's the regression to fix before doing anything else.

...
- **What does NOT exist yet**: event-triggered threads (mail/calendar; no extension calls `emitEvent` yet). The §06 severity gate + policy management UI are now in place (users can create/revoke policies via the "Autonomi" sidebar). The per-tool redactor pipeline is wired host-side, but extension-registered tools still hit the no-redactor branch because redactor functions can't cross the worker IPC boundary in v1. Remaining gaps: Escalate and Solve-differently collision options, extension-IPC redactor support, severity-change cascade handling, and Stina-suggested policies.
...
```sh
pnpm test                    # full suite, currently 497 tests
...
Should print 497 tests passed, no typecheck errors. If either fails, that's the regression to fix before doing anything else.

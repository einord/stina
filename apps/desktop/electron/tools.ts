...
export const toolSpecs = {
  openai: [{
    type: 'function',
    function: {
      name: 'console_log', description: 'Log a message to the Stina console for debugging/observability.',
      parameters: { type: 'object', properties: { message: { type: 'string' } }, required: ['message'] }
    }
  },{
    type: 'function',
    function: {
      name: 'mcp_call', description: 'Call a tool on an external MCP server over WebSocket.',
      parameters: { type: 'object', properties: { server: { type: 'string' }, tool: { type: 'string' }, args: { type: 'object' } }, required: ['server','tool'] }
    }
  },{
    type: 'function',
    function: {
      name: 'mcp_list', description: 'List available tools on an external MCP server.',

...
const electronAPI = {
  ...
  chatEventsSubscribe: (handler: (event: { type: 'instruction-received' | 'conversation-updated'; userId: string; conversationId?: string; payload?: Record<string, unknown> }) => void): (() => void) => {
    const listener = (_event: Electron.IpcRendererEvent, payload: { type: 'instruction-received' | 'conversation-updated'; userId: string; conversationId?: string; payload?: Record<string, unknown> }) => {
      handler(payload)
    }
